    ...
    <div class="managmenet_container">
        <div class="flex flex-col  border-b-2 p-2">
            <div class="flex  justify-between items-center">
                ...
                <input v-model="search" @change="getReviews(null)" class="relative block max-w-[5rem] md:max-w-fit  px-3 py-2 border border-gray-300 placeholder-gray-500
                text-gray-900 rounded-md
                     focus:outline-none
                     focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Search">
            </div>
            <span class="mt-4">Found {{ reviews.total }} reviews</span>
        </div>
        <table class="table_tag">
            ...
            <tr>
                ...
                <TableHeadingCell @click="sortProduct" class="border-b-2 p-2 text-left" field="tour_id"
                                  :sort-field="sortField" :sort-direction="sortDirection">
                            ...
                            <MenuItems
                                ...
                                <div class="px-1 py-1">
                                    <MenuItem v-slot="{ active }">
                                        <RouterLink :to="{ name: 'app.reviews.edit', params: { id: review.id } }"
                                                    :class="[
        ...
        <div v-if="!reviews.loading" class="justify-between items-center mt-5 md:flex p-3">
            <span>Showing From {{ reviews.from }} to {{ reviews.to }}</span>
            <nav v-if="reviews.total > reviews.limit"
                 class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px md:flex "
                 aria-label="pagination">
                <a v-for="(link, i) of reviews.links" :key="i" :disabled="!link.url" href="#"
...
function deleteReview(id) {
    ...
    store.dispatch('deleteReview', id)
        .then(res => {
            store.commit('showToast', 'Review Deleted Successfully')
            store.dispatch('getReviews')
        })
}
