...
function onSubmit($event, close = false) {
    loading.value = true
    store.dispatch('createBlogTranslation', blog.value)
        .then(response => {
            loading.value = false;
...
onMounted(() => {
    ...
    if (route.params.id) {
        Promise.all([
            store.dispatch('getBlogForTranslation', route.params.id),
            // store.dispatch('getCities')
        ])
            .then(([blogResponse, citiesResponse]) => {
