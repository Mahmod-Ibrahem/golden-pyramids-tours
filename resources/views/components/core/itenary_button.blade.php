<div id="title_{{$id}}" class="flex justify-between items-center cursor-pointer transition-all rounded-lg h-fit shadow-md border-l-[3px] border-l-main border-b
 mx-auto bg-white mt-3">
    ...
    <svg id="plus_{{$id}}" class="h-6 w-6 md:h-8  md:w-8 flex-shrink-0 text-bg-main" fill="none" viewBox="0 0 24 24"
         stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
    </svg>

    <svg id="Minus_{{$id}}" class="h-6 w-6 md:h-8  md:w-8 hidden flex-shrink-0 text-bg-main" width="24" height="24"
...
<div id="descr_{{$id}}"
    ...
    <div class="py-3 px-4 font-semibold pl-3 m tracking-wide leading-5 whitespace-normal  break-words bg-white text-xs md:text-[0.90rem] capitalize
                 prose max-w-none prose-a:no-underline prose-a:!text-bg-main !text-black prose-p:!text-black">
        {!! $itenary !!}
    </div>
