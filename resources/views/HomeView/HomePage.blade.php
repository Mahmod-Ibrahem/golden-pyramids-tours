...
@section('content')
    ...
    <div class="h-[25rem] md:h-screen w-full ">
        <div
            ...
            <div class="flex flex-col items-center justify-center  w-full h-full bg-black/10 pt-10">
                ...
                {!! $pageText['homeTitle']->getTranslation('content',app()->getLocale()) !!}
                </div>
            </div>
        </div>
    </div>
    ...
    @component('HomeView.Services',['content'=>$pageText['services']->getTranslation('content',app()->getLocale())])@endcomponent
    @if(!$reviews->isEmpty())
    @component('HomeView.Testimonials',['reviews' => $reviews])@endcomponent
    @endif
    @component('components.Sections.TailorMade',['tailorText'=>$pageText])@endcomponent
    ...
    @if($recommendedTourPackages ?? false)
        @component('HomeView.RecommendedDayTours',['tours' => $recommendedTourPackages])@endcomponent
    @endif
